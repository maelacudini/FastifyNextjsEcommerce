...
export async function userController( fastify: FastifyInstance ) {
	...
	fastify.get<{
		...
		const data = await usecasesFactory.findAllUsersUseCase.execute( req.query )
	...
	fastify.get<{
		Params: FindUserByIdRequestType,
	...
	fastify.get<{
		Params: FindUserByEmailRequestType,
	...
	// For update endpoints, the correct split is:
	// - params holds the resource identifier when the caller is targeting another user: PATCH /user/:id
	// - body holds only the fields to mutate
	// - the id should never be accepted in the body
	// - for self-service updates, the client should not send any user id at all: use PATCH /user/me and derive the target user from the JWT sub
