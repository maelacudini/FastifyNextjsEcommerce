...
### pgAdmin connection settings (inside Docker Compose)

When adding a server in pgAdmin:
- Host name/address: `postgres` (not `localhost`)
- Port: `5432`
- Username: value of `POSTGRES_USER`
- Password: value from `secrets/postgres_password.txt`
- Database: value of `POSTGRES_NAME`

`localhost` from inside pgAdmin means "the pgAdmin container itself", not your Postgres container.

### If you see `relation "users" does not exist`

The app expects a `users` table. This repository includes an init script at `docker/postgres/init/001_schema.sql` that creates it automatically on first DB initialization.

If your `postgres_data` volume already existed before adding the init script, Postgres will not re-run init files. Reset volumes once:
...
> Warning: `down -v` deletes local Docker DB data for this project.
